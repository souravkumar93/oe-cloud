...
module.exports.checkModelWithPlural = function utilCheckModelWithPlural(server, plural) {
  ...
  for (var i = 0; i < models.length; ++i) {
    if (models[i].pluralModelName === plural) {
      return models[i].modelName;
    }
  }
...
module.exports.createModel = function utilCreateModel(app, modeldefinition, options, cb) {
  ...
  model.updateId = uuid.v4();

  // var model = loopback.createModel(JSON.parse(JSON.stringify(modeldefinition)));
  app.locals.modelNames[model.modelName.toLowerCase()] = model.modelName;
  app.locals.modelNames[model.modelName] = model.modelName;
...
module.exports.isModelDefinitionValid = function isModelDefinitionValid(modeldefinition, options, cb) {
  ...
  fnArr = fnArr.concat(fnArr, getPropertyValidityFns(modeldefinition, options), getEvValidationValidityFns(modeldefinition, options));
...
/**
 *
 * Aggregation of expressions(mustache queries and expression language expressions) attached to all the evValidations object of the model
 ...
 * @returns {Object[]} Array containing all the functions that will evaluate the expressions present in evValidations
 */
function getEvValidationValidityFns(modeldefinition, options) {
  var expr = {};
  var evRulesFn = [];
  var validations = modeldefinition.evValidations || {};
  log.info(options, 'aggregating EvValidation validation rules for : ', modeldefinition.name);
  Object.keys(validations).forEach(function modelDefinitionValidationsForEachCb(evValidationName) {
    var evValidation = validations[evValidationName];
    var path = modeldefinition.name + '->' + evValidationName + '->';
    ...
    if (expression) {
      ...
      // pick the evValidation rule i.e. refWhere in case of reference type rule or expression in case of custom type rule
      expr = {
        rule: evValidation.expression || evValidation.refWhere,
        type: evValidation.type,
        path: path + 'rule'
      };
      // this wrapper prepares an array of functions to evaluate the validity of a model
      evRulesFn.push(async.apply(expressionCheck, expr, options));
...
function getPropertyValidityFns(modeldefinition, options) {
  ...
  log.info(options, 'aggregating property level rules for : ', modeldefinition.name);
  ...
  switch (type) {
    case 'reference':
      try {
        ...
        log.error(log.defaultContext(), 'checking validity of mustache query passed in EvValidation : ', path);
    ...
    case 'custom':
      try {
        ...
        log.error(log.defaultContext(), 'checking validity of expression language query passed in EvValidation : ', path);
        expressionValidity = false;
      }
      break;
    default:
