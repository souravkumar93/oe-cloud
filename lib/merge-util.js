 ...
 * @param {String} appRootDir - path of application root directory
 * @param {String} env Environment, usually `process.env.NODE_ENV`
 * @param {String} name - filename
 ...
 * @param {Array.<String>} files - config files
 * @returns {Array.<Object>} - config
 */
...
var loadAppList = function loadAppList(applist, dir, bootoptions) {
  ...
  applist.forEach(function applistForEachFn(appitem, appIndex) {
    if (appitem.enabled !== false) {
      ...
      if (models._meta && models._meta.sources) {
        models._meta.sources.forEach(function modelsSourcesForEachFn(element, idx, arr) {
      ...
      if (models._meta && models._meta.mixins) {
        models._meta.mixins.forEach(function modelsMixinsForEach(element, idx, arr) {
          // var abspath = path.resolve(path.join(serverDir, element));
          // var relpath = path.relative(evfPath, abspath);
          var p1;
          if (!isCurrentModule) {
      ...
      var modobj = {};
      Object.keys(middlewares).forEach(function middlewaresForEachFn(phase) {
        var mlist = middlewares[phase];
        if (Object.keys(mlist).length > 0) {
          modobj[phase] = {};
          Object.keys(mlist).forEach(function mlistForEachn(key) {
            var newKey = key;
            if (!isCurrentModule) {
      ...
      if (fs.existsSync(providerJsonPath)) {
        ...
        // var providerConfig = require(providerJsonPath);
